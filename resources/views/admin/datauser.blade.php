...
@section('content')
    <div id="removeUserModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full">
        <!-- Modal content -->
        <div class="container mx-auto p-4 max-w-2xl mt-24">
            <div class="bg-white rounded shadow">
                <div class="border-b p-4">
                    <h5 class="font-bold uppercase text-gray-600">Remove User</h5>
                </div>
                <div class="p-4">
                ...
                <div class="flex justify-end p-4 border-t">
                    <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded m-2"
                        onclick="document.getElementById('removeUserForm').submit();">Yes, Remove</button>
                    <button class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded m-2"
                        onclick="document.getElementById('removeUserModal').classList.add('hidden');">Cancel</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="container grid px-6 mx-auto">
                        ...
                        <tr class="text-gray-700 dark:text-gray-800">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <a href="{{ route('data-user.show', $post->id) }}"
                                    ...
                                    <a href="{{ route('data-user.edit', $post->id) }}"
                                    ...
                                    <form action="{{ route('data-user.destroy', $post->id) }}" method="post"
                                        ...
                                        <button
                                            ...
                                            aria-label="Delete"
                                            onclick="event.preventDefault(); showRemoveModal('{{ route('data-user.destroy', $post->id) }}')">
                                            <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
        ...
        <script>
            ...
            function showRemoveModal(actionUrl) {
                document.getElementById('removeUserForm').action = actionUrl;
